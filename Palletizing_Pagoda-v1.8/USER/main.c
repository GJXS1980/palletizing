...
/*			
			if(time_cnt>200*120) //300S/60 = 5min
			{
				//超时处理 故障信息
	...
	{		
		...
		if(ack_cmd) 							//发送确认消息
		{
			switch(recei_pall_data.cmd_type)
			{
				...
				case 1: 	current_location_send();break;	//获取码垛当前位置
				
				case 2: 	TIM_Cmd(TIM8, ENABLE);
									TIM_Cmd( TIM13, ENABLE );
                  TIM_Cmd(TIM3,  ENABLE );									
									ack_send();break;    						//正常操作指令  开始
				
				case 3: 	TIM_Cmd(TIM8, DISABLE);
									TIM_Cmd( TIM13, DISABLE );
									TIM_Cmd(TIM3,  DISABLE );
									ack_send();break;    						//暂停
				...
				case 5: 	PALL_U_IO_Control(1); 
									ack_send();break;    						//夹
				case 6: 	PALL_U_IO_Control(0); 
									ack_send();break;    						//放									
				case 7: 	PALL_Origin(1); 
									ack_send();break;    						//X回原点
				case 8: 	PALL_Origin(2);
									ack_send();break;    						//Y回原点									
				
				default : error_ack_send();   					 //其它错误指令
		...
		switch(KEY_Scan(0))
		{
			case START_PRES: TIM_Cmd(TIM8, ENABLE);TIM_Cmd( TIM13, ENABLE );TIM_Cmd(TIM3,  ENABLE );
											 RED_OFF;GREEN_ON; YELLOW_OFF;
											 SUSPEND_STATUS = 0;break;			//启动
			
			case PAUSE_PRES: TIM_Cmd(TIM8, DISABLE);TIM_Cmd( TIM13, DISABLE );TIM_Cmd(TIM3,  DISABLE );
											 RED_OFF;GREEN_OFF;YELLOW_ON;
											 SUSPEND_STATUS = 1;break;		//暂停
			default :break;			
